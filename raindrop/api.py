...
def create_gl_entries():
    ...
    for pay in payment_entries:
        if not frappe.db.exists("GL Entry", {"voucher_no":pay.name}):
            ...
            create_gl_entry_debit(pay.posting_date,pay.paid_from, pay.cost_center, pay.paid_amount, pay.paid_from_account_cureency, pay.party, pay.name, pay.project, pay.reference_no, pay.company)
