    ...
    with open( '/home/frappe/frappe-bench/apps/raindrop/Purchase Invoice 2020 to 2023 updated infor number - backup bill 2.0 final.csv') as design_file:
        ...
        for value in  reader_po:
            try:
                ...
                taxes = []
                tax_template = []
                with open('/home/frappe/frappe-bench/apps/raindrop/Purchase Invoice 2020 to 2023 updated infor - backup bill 2.0 final.csv') as templates:
                    reader = csv.reader(templates, delimiter=',')
                    for row in reader:
                        cost_center = "Main - HPL"
                        if row[45] != '':
...
def create_missing_purchase_invoice_2020_from_po():
    with open( '/home/frappe/frappe-bench/apps/raindrop/Correct purchase invoice 2024 Number 06_03_24 - Sheet1.csv') as design_file:
        reader_po = csv.reader(design_file, delimiter=',')
        for value in  reader_po:
            try:
                if not frappe.db.exists("Purchase Invoice", {"custom_internal_id":value[0]}):
                    items = []
                    taxes = []
                    tax_template = []
                    po_name = frappe.db.get_value("Purchase Order", {"custom_document_number":value[52]}, 'name')
