...
def create_gl_entries():
    ...
    for pay in payment_entries:
        if not frappe.db.exists("GL Entry", {"voucher_no":pay.name}):
            create_gl_entry_credit(pay.posting_date, pay.paid_to, pay.cost_center, pay.paid_amount, pay.paid_from_account_cureency,  pay.party_type, pay.party, pay.paid_from, pay.name, pay.project, pay.reference_no,  pay.company)
            create_gl_entry_debit(pay.posting_date,pay.paid_from, pay.cost_center, pay.paid_amount, pay.paid_to_account_cureency, pay.party, pay.name, pay.project, pay.reference_no, pay.company)
...
def create_goods_received_2023():
    with open('/home/frappe/frappe-bench/apps/raindrop/Purchase received Number  - Sheet1.csv' ) as design_file:
        reader_po = csv.reader(design_file, delimiter=',')
        for value in  reader_po:
            try:
...
def create_purchase_invoice_2020():
    with open( '/home/frappe/frappe-bench/apps/raindrop/Purchase_Invoice 2024 Number - Sheet1.csv') as design_file:
        reader_po = csv.reader(design_file, delimiter=',')
        for value in  reader_po:
            try:
                items = []
                taxes = []
                tax_template = []
                with open('/home/frappe/frappe-bench/apps/raindrop/Purchase_Invoice 2024 - Sheet1.csv' ) as templates:
                    reader = csv.reader(templates, delimiter=',')
                    for row in reader:
                        cost_center = "Main - HPL"
                        if row[45] != '':
